...
class WialonService
{
    ...
    public static function getAllBusesLocation2() {
        ...
        // Agar ma'lumot olishda xato bo'lsa, yangi sessiya ochish va qayta urinish
        if ($data === false) {
            self::createSession();
            $data = self::fetchData();
        }
    ...
    private static function createSession() {
        // Kirish so'rovini yuborish va sessiya yaratish
        $loginResponse = Http::get(self::$apiUrl, [
            'svc' => 'token/login',
            'params' => json_encode(['token' => self::$token])
        ]);
        ...
        if ($loginResponse->successful()) {
            self::$sessionId = $loginResponse->json()['eid'];
        } else {
            // Log yoki xato qayd etish
            self::$sessionId = null;
        }
    }

    private static function isSessionValid() {
        // Sessiya ID mavjudligini tekshirish
        return isset(self::$sessionId);
    }

    private static function fetchData() {
        // Ma'lumot olish so'rovini yuborish
        $response = Http::get(self::$apiUrl, [
